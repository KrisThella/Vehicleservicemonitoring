...
```
server/
  db.ts                 # SQLite connection + schema (vehicles, prices, profile,
                        #   settings, pull_outs, payments, inventory_rows)
  seed.ts               # First-run seed data (7 vehicles, 19 prices, 1 profile,
                        #   6 pull-outs, 4 payments, 4 inventory rows)
  index.ts              # Express REST API on 127.0.0.1:3001 (uses cors)
src/
  main.tsx              # Entry — bootstraps theme from localStorage
  lib/
    api.ts              # Fetch hooks: useVehicles, usePrices, useProfile,
                        #   usePullOuts, usePayments, useInventory(year)
  app/
